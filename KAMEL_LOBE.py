...
def KAMEL_LOBE(r,RDF,w=0.015):
    ...
    m_KL = np.ceil(2*w/delr) # number of bins to average over
    ...
    if m_KL > 1:
        ...
        M1[0,:] = 0
        np.arange(0, Nbins)**2
        M2 = np.diag(np.arange(0, Nbins)**2)*delr**2 # 
        T1 = 2*np.pi*delr*M1@M2

        k_KL = 2*m_KL-1
        #T2 = np.identity(Nbins)
        fractions = np.zeros((1,k_KL))

        A1_block = np.identity(Nbins)
        A2_block = np.zeros((m_KL, Nbins-m_KL))
        #B_block = T2[m_KL:-m_KL,:]
        ...
        B_block = sp.diags(np.tile(fractions, (Nbins-2*m_KL, 1)), np.arange(0, 2*(m_KL-1)+1), shape=(Nbins-2*m_KL, Nbins)).toarray()
        ...
        K = np.ones((Nbins,1)) * (-1)**(np.arange(1,Nbins)+1)
        K_sq = np.oiuter(K,K)
        KTr = 2*np.tril(K_sq)
